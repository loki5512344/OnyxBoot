...
struct FAT32 {
    ...
    bool init(blk_read_t r, void* p, uint64_t plba) {
        read = r; priv = p; part_lba = (uint32_t)plba;
        uint8_t bpb[512];
        if (!read(part_lba, bpb, priv)) return false;
        if (bpb[510] != 0x55 || bpb[511] != 0xAA) return false;
        if (rd16(bpb + 11) != 512) return false;
        sec_per_clus = bpb[13];
        rsvd_sec = rd16(bpb + 14);
        fat_cnt = bpb[16];
        root_clus = rd32(bpb + 44);
        fat_sec = rd32(bpb + 36);
        data_sec = part_lba + rsvd_sec + fat_cnt * fat_sec;
        return sec_per_clus != 0 && fat_cnt != 0;
    }

    uint32_t next_clus(uint32_t clus, uint8_t* tmp) {
        uint32_t fat_off = clus * 4;
        if (!read(part_lba + rsvd_sec + fat_off / 512, tmp, priv)) return 0;
        uint32_t val = rd32(tmp + fat_off % 512) & 0x0FFFFFFF;
        return (val >= 0x0FFFFFF8) ? 0 : val;
    }

    bool read_clus(uint32_t clus, uint8_t* buf, uint8_t*) {
        uint32_t lba = data_sec + (clus - 2) * sec_per_clus;
        for (uint32_t i = 0; i < sec_per_clus; i++)
            if (!read(lba + i, buf + i * 512, priv)) return false;
        return true;
    }

    bool find_file(const char name[11], uint32_t* clus, uint32_t* size, uint8_t* tmp) {
        uint32_t cur = root_clus;
        while (cur) {
            if (!read_clus(cur, tmp, tmp + 512)) return false;
            for (uint32_t i = 0; i < sec_per_clus * 512; i += 32) {
                uint8_t* e = tmp + i;
...
bool fat32_read_file(blk_read_t read, void* priv, const char* name,
    ...
    FAT32 fs;
    if (!fs.init(read, priv, plba)) return false;
    char name83[11];
    name_to_83(name, name83);
    uint8_t tmp[1024];
    uint32_t fclus, fsize;
    if (!fs.find_file(name83, &fclus, &fsize, tmp)) return false;
    if (fsize > max_size) fsize = max_size;
    *size = fsize;
    uint32_t off = 0;
    while (fclus && off < fsize) {
        if (!fs.read_clus(fclus, buf + off, tmp)) return false;
        off += fs.sec_per_clus * 512;
        fclus = fs.next_clus(fclus, tmp);
    }
    return true;
}
